    ...
    for idx = 1, neg_sample_lookup_size do
        ...
        if idx / vocab_size > word_prob then
            ...
            if word_index > vocab_size then
                if lookup_upper_limit == 0 then
                    print("here")
                    print(idx)
                    lookup_upper_limit = idx
                end
            	word_index = word_index - 1
        	end
...
function generate_neg_samples(node)
    ...
    while i <= num_neg_samples do
        neg_context = neg_sample_lookup[torch.random(lookup_upper_limit+1)]
        local flag = 1
...
build_table()
print(neg_sample_lookup[1e5 - 1])
print(lookup_upper_limit)
...
node_lookupR1 = nn.LookupTable(vocab_size, node_embed_size)
model_FOP = nn.Sequential()
model_FOP:add(nn.ParallelTable())
model_FOP.modules[1]:add(node_lookupR1)
model_FOP.modules[1]:add(node_lookupR1:clone('weight', 'bias', 'gradWeight', 'gradBias'))
...
model3.modules[1]:add(node_lookupR2)
...
embed1 = node_lookupR1.weight
embed2 = node_lookupR2.weight
...
for i=1,#vocab do
    ...
    for j=1,node_embed_size do
        ...
        file:write(embed1[i][j])
    end
    file:write(sep)
    for j=1,node_embed_size do
        if j>1 then file:write(sep) end
        file:write(embed2[i][j])
    end
    file:write('\n')
end
file:close()
